...
class MeloFormDataset(LanguagePairDataset):
    def __init__(
        self,
        src,
        src_sizes,
        ...
        self.symbol_token = self.src_dict.index('|')
    ...
    def __getitem__(self, index):
        ...
        div_positions  = [
            i for i, x in enumerate(src_list) if x == self.div_token
        ]

        # extract repetition patterns
        src_patterns = src_list[sep_positions[-1] + 1:]
        patterns = []
        symbol_positions = [
            i for i, x in enumerate(src_patterns) if x == self.symbol_token
        ]
        symbol_positions.insert(0, -1)
        for sym_i in range(len(symbol_positions) - 1):
            patterns.append([int(self.src_dict[x]) for x in src_patterns[symbol_positions[sym_i] + 1:symbol_positions[sym_i + 1]]])

        # extract src list
        src_list = src_list[:sep_positions[-1] + 1]
        selected_seg_idxes = random.choice(patterns)
        ...
        for i in range(len(sep_positions)-1):
            if i in selected_seg_idxes:
                sent = src_list[sep_positions[i] + 1: div_positions[i]] + [self.src_dict.index('[sep]')]
                sent = [x for x in sent if 'TREND' not in self.src_dict[x]]
                tgt = src_list[div_positions[i] + 1: sep_positions[i + 1]] + [self.tgt_dict.index('[sep]')]
                target.extend(tgt)
                target_sent_ids.extend([i] * len(tgt))
            else:
                sent = src_list[div_positions[i] + 1: sep_positions[i + 1] + 1]
            source.extend(sent)
            source_sent_ids.extend([i] * len(sent))

        target = target[:-1]
        target_sent_ids = target_sent_ids[:-1]
