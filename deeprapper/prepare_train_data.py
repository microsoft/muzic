...
def get_pinyin_finals(lines):
    """
    lines[list]: several lines of strings. Examples shown in main(). 
    here would be lyric of one song.
    """
    all_finals = []
    for line in lines:
        all_finals.append(get_sentence_pinyin_finals(line))
    
    return all_finals


special_tokens = ['[SEP]', '[MASK]', '[PAD]', 
                  '[CLS]', '[UNK]', '[BEAT]', 
                  '[S]', '[F]', '[M]']
...
def build_single_file(raw_data_path, tokenized_data_path, full_tokenizer, num_pieces, segment=False):
    """
    如果训练材料是全部堆在一起不分篇章的话用这个函数
    :param raw_data_path:
    :param tokenized_data_path:
    :param full_tokenizer:
    :param num_pieces:
    :return:
    """
...
""" Prepare Wiki_zn Data """
def read_wiki_zn(path='data/wiki_zh/', output_path='data/train_wiki.json'):
    ...
    # to merge all articles in a list
    lines = []

    # sub dirs in wiki_zh: AA  AB  AC  AD  AE  AF  AG  AH  AI  AJ  AK  AL  AM
    for dp in os.listdir(path):
        dp = os.path.join(path, dp)
        print(f'loading dir: {dp}')
        if os.path.isdir(dp):
            # enumerate each file in one subdir
            for fp in os.listdir(dp):
                fp = os.path.join(dp, fp)
                if os.path.isfile(fp):
                    with open(fp) as inps:
                        # enumerate each line in the file
                        # each line is an article
                        for l in inps:
                            # 用[EOL]表示换行
                            text = json.loads(l)["text"].replace("\n", "[EOL]")
                            lines.append(text)
        #                     break
        #     # TODO just load one file for test at now
        #     break
        # break

    # save caches
    json.dump(lines, open(output_path, mode='w'))

    return lines


""" Prepare Poetry Data """
def read_poetry(path='data/poetry.json.txt', out_path='data/train_poetry.json.txt'):
    # check whether preprocessed cache exists or not
    print('Start to read processed poetry from dataset....')
    lines = []
    if os.path.exists(out_path):
        # load cached data
        with open(out_path, encoding='utf8') as ins:
            for l in ins:
                lines.append(l)
        return lines
    ...
    # to merge content of all poetry in a list
    lines = []
    with open(path, encoding='utf8') as ins:
        # enumerate each line in the file
        # each line is an article
...
def check_valid(content):
    test_str = re.findall(r'\W', content)
    if test_str == None:
        return True
    for s in test_str:
...
def parse_poetry(a_path, poetry_type):

    with open(a_path, encoding='utf8') as f:
        lines = json.load(f)
        newlines = []
        for i in range(len(lines)):
            line = []
            lines[i]['type'] = poetry_type
            
            if 'paragraphs' in lines[i]:
                tmp = lines[i]['paragraphs']
                for content in tmp:
                    content = content.rstrip(''.join(valid_codes))
                    if check_valid(content):
                        line.append(content)
                if len(line):
                    lines[i]['paragraphs'] = '。'.join(line) + '。'
...
def prepare_poetry(ins_path='data/poetry/', out_path='data/poetry.json.txt'):
    print('Start to process poetry....')
    poems = []
    # enumerate subdir in data/poetry/
    i = 0  # total num
    for s_path in os.listdir(ins_path):
        ...
        if os.path.isdir(s_path):
            ...
            for a_path in os.listdir(s_path):
                # print(a_path)
                file_type = a_path.split('.')[-1]
                file_head = a_path.split('.')[0]
                a_path = os.path.join(s_path, a_path)
                
                if file_type != 'json' or file_head == 'author' or file_head == 'authors':
                    continue
                    
                lines = parse_poetry(a_path, poetry_type)
                poems.extend(lines)
                i += 1
                if i % 100 == 0:
    ...
    print(f'End. Total Processed Poems:  {len(poems)}')
                                ...
                                for l_song in os.listdir(l_path):
                                    ...
                                    if with_beat:
                                        if beat_mode == 0:
                                            ...
                                            if l_song != 'final_with_beat_local.txt':
                                                continue
