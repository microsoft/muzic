...
for i in range(0, len(dataset), batch_size):
    if n_samples and i == group_size * n_samples:
        break
    target = np.vstack(tuple(dataset[j]['target'].numpy() for j in range(i, i + batch_size) if j < len(dataset)))
    target = torch.from_numpy(target)
    target = F.one_hot(target.long(), num_classes=num_classes)
    target = target.sum(dim=1)
    source_batch_max_length = max(dataset[j]['net_input.src_tokens'].size(0) for j in range(i, i + batch_size) if j < len(dataset))
    source = np.vstack(tuple(padded(dataset[j]['net_input.src_tokens'].numpy(), source_batch_max_length) for j in range(i, i + batch_size) if j < len(dataset)))
    source = torch.from_numpy(source)
    output = F.softmax(roberta.predict('acc_head' if 'acc' in sys.argv[1] else 'next_head', source, True), dim=1)
    y_true.append(target.detach().cpu().numpy())
    y_pred.append(output.detach().cpu().numpy())
    print('evaluating: {:.4f}%'.format(i / len(dataset) * 100), end='\r', flush=True)
...
for z in [1, 5, 10, 15, 20, 25]:
    print('HITS@{}: {:.6f}'.format(z, sum(sum(q[:z]) / sum(q) for q in query_list) / len(query_list)))
