    ...
    def load_dataset(self, split, combine=False, **kwargs):
        ...
        with open(os.path.join(self.args.data, 'label', split+".label")) as file:
            for line in file:
                line = line.strip()
                line = line.split()
                label = [self.label_dictionary.index(l) for l in line]
...
class OctupleTokenDataset(PrependTokenDataset):
    ...
    def convert(self, item):
        ...
        if convert_encoding == 'CP':
            encoding = list(3 if j == None else j for i in encoding for j in i)[
                :crop_length * 8]
        elif convert_encoding == 'REMI':
            encoding = list(j for i in encoding for j in i if j != None)[
                :crop_length]
        else:
            assert False, 'Unknown encoding format'
        encoding = ([0] * 8) + encoding + ([2] * 8)
