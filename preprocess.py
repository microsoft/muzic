...
def gen_dictionary(file_name):
    ...
    with open(file_name, 'w') as f:
        for i in range(bar_max): print('<0-{}>'.format(i), num, file=f)
        for i in range(beat_note_factor * max_notes_per_bar * pos_resolution): print('<1-{}>'.format(i), num, file=f)
        for i in range(129): print('<2-{}>'.format(i), num, file=f)  # 128 for percussion
        for i in range(256): print('<3-{}>'.format(i), num, file=f)  # 128~255 for percussion
        for i in range(duration_max * pos_resolution): print('<4-{}>'.format(i), num, file=f)
        for i in range(v2e(127) + 1): print('<5-{}>'.format(i), num, file=f)
        for i in range(len(ts_list)): print('<6-{}>'.format(i), num, file=f)
        for i in range(b2e(max_tempo) + 1): print('<7-{}>'.format(i), num, file=f)
...
def MIDI_to_encoding(midi_obj):
    ...
    pos_to_info = [[None for _ in range(4)] for _ in range(max_pos)]  # (Measure, TimeSig, Pos, Tempo)
    ...
    for j in range(len(pos_to_info)):
        if pos_to_info[j][1] is None:
            pos_to_info[j][1] = t2e(time_signature_reduce(4, 4))  # MIDI default time signature
    ...
    for inst in midi_obj.instruments:
        for note in inst.notes:
            ...
            encoding.append((info[0], info[2], 128 if inst.is_drum else inst.program, note.pitch + 128 if inst.is_drum else note.pitch, d2e(time_to_pos(note.end) - time_to_pos(note.start)), v2e(note.velocity), info[1], info[3]))
    if len(encoding) == 0:
        return list()
...
def encoding_to_MIDI(encoding):
    ...
    midi_obj.instruments = [miditoolkit.containers.Instrument(program=(0 if i == 128 else i), is_drum=(i == 128), name=str(i)) for i in range(128 + 1)]
    for i in encoding:
        ...
        velocity = e2v(i[5])
        midi_obj.instruments[program].notes.append(miditoolkit.containers.Note(start=start, end=end, pitch=pitch, velocity=velocity))
    ...
    for i in range(len(bar_to_timesig)):
        ...
        if new_ts != cur_ts:
            ...
            midi_obj.time_signature_changes.append(miditoolkit.containers.TimeSignature(numerator=numerator, denominator=denominator, time=get_tick(i, 0)))
            cur_ts = new_ts
    cur_tp = None
    for i in range(len(pos_to_tempo)):
...
def F(file_name):
    ...
    try:
        ...
        assert all(0 <= j.start < 2 ** 31 and 0 <= j.end < 2 ** 31 for i in midi_obj.instruments for j in i.notes), 'bad note time'
        assert all(0 < j.numerator < 2 ** 31 and 0 < j.denominator < 2 ** 31 for j in midi_obj.time_signature_changes), 'bad time signature value'
    ...
    try:
        ...
        for p in range(0 - random.randint(0, sample_len_max - 1), len(e), sample_step):
            ...
            bar_index_list = [e[i][0] for i in range(L, R + 1) if e[i][0] != None]
            bar_index_min = 0
            bar_index_max = 0
            ...
            bar_index_offset = random.randint(offset_lower_bound, offset_upper_bound) if offset_lower_bound <= offset_upper_bound else offset_lower_bound  # to make bar index distribute in [0, bar_max)
            e_segment = []
            for i in e[L: R + 1]:
                if i[0] == None or i[0] + bar_index_offset < bar_max:
                    e_segment.append(i)
                else:
                    break
