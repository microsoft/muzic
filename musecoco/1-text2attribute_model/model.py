...
class MusicBertEmbeddings(BertEmbeddings):
...
class MusicBertModel(BertModel):
    def __init__(self, config, num_labels, tokenizer, add_pooling_layer=True):
        ...
        self.embeddings = MusicBertEmbeddings(config, num_labels, tokenizer)
...
class SequenceClassifierOutputForMultiTask(ModelOutput):
    ...
    Args:
        ...
        logits (The element of `Dict` is in shape `(batch_size, config.num_labels)`):
        ...
        attentions (`tuple(torch.FloatTensor)`, *optional*, returned when `output_attentions=True` is passed or when `config.output_attentions=True`):
            ...
            heads.
    """
    # diff_loss: Optional[Dict[str, torch.FloatTensor]] = None
