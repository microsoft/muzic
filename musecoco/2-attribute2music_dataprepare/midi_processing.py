from midiprocessor import midi_utils, MidiEncoder, enc_remigen_utils
...
def convert_pos_info_to_tokens(encoder, pos_info):
    pos_info_id = encoder.convert_pos_info_to_pos_info_id(pos_info)
    tokens = enc_remigen_utils.convert_pos_info_to_token_lists(
        pos_info_id, ignore_ts=False, sort_insts='id', sort_notes=None,
    )[0]
    tokens = enc_remigen_utils.convert_remigen_token_list_to_token_str_list(tokens)
    return tokens
...
if __name__ == '__main__':
    ...
    #     local_pos: bar内的onset位置，例如在一个4拍的bar中，该数字会从1一直到48
