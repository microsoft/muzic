...
from config import PUNC, WEAK_BEAT, TICKS
...
def get_pitch_score(song:Song):
    ...
    for i, l in enumerate(lyrics):
        ...
        # Contour
        try:
            # We regard the last note of the word as the last note of the sents,
            # by getting the note index of next word and minus 1, we could get 
            # the last note of current word
            if l[-1] in PUNC:
                try:
                    curr_pit_id = song.get_noteidx_by_time(song.lyric["time"][i+1]) - 1
                except IndexError:
                    curr_pit_id = song.n_notes
                curr_pitch  = song.get_pitch(curr_pit_id)
                
                # ic(i, l)
                # ic(first_pitch, curr_pitch, curr_pit_id)
                
                if l[-1] in ['.'] and curr_pitch < first_pitch or \
                   l[-1] in [',', '?'] and curr_pitch > first_pitch:
                    contour += 1
                
                # first pitch would be the first note of the next sentence
                first_pitch = song.get_pitch(curr_pit_id+1)
    ...
    pitch_detail = [contour_score]
    pitch_score  = contour_score
...
if __name__ == "__main__":
    score = get_score("testmid/en/tele-en")
    ic(score)