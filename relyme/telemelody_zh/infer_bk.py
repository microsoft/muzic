...
from preprocess_chord_16 import v2e, t2e, b2e, encoding_to_MIDI, pos_resolution
from magenta_chord_recognition import _PITCH_CLASS_NAMES, _CHORD_KIND_PITCHES, NO_CHORD
from config import GEN_MODE, TEMPO

logging.basicConfig(level=logging.DEBUG)
PRETRAIN = "bert-base-chinese"
L2B_MODEL = 'checkpoints/seq2seq-bert-base-chinese_4-2/model.pt'
...
config = BertConfig.from_pretrained(PRETRAIN, add_pooling_layer=False)
encoder = BertGenerationEncoder.from_pretrained(PRETRAIN,
                                                bos_token_id=BOS,
                                                eos_token_id=EOS,
                                                num_hidden_layers=4)
...
decoder_config.pad_token_id = DEC_PAD
decoder_config.bos_token_id = DEC_BOS
decoder_config.eos_token_id = DEC_EOS
decoder_config.num_hidden_layers = 2
decoder_config.vocab_size = DEC_VOC + 3
decoder_config.num_attention_heads = 4
decoder = BertGenerationDecoder(decoder_config)

model = EncoderDecoderModel(encoder=encoder, decoder=decoder)
model.load_state_dict(torch.load(L2B_MODEL, map_location=torch.device('cpu')))
model.eval()
...
def stage_1_get_l2b(lyrics):
    ...
    tokenizer_outputs = tokenizer(' '.join(lyrics), return_tensors='pt')
    pred_cnt = 0
    while pred_cnt < 30:
        pred = model.generate(input_ids=tokenizer_outputs.input_ids,
                            do_sample=True,
                            temperature=0.5,
                            top_k=2,
                            min_length=0,
                            max_length=len(lyrics))

        start_label = [ int(i) for i in pred.tolist()[0] if i not in (DEC_BOS, DEC_EOS) ]
        pred_cnt += 1
        print(pred_cnt)
