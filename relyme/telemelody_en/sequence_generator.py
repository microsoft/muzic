...
from en_constraintor import TokenConstraintor
...
class SequenceGenerator(nn.Module):
    def __init__(
        ...
        self.strct = strct
        self.sents_form = sents_form
        self.in_word_pos = in_word_pos
        self.TOKEN = _config.TOKEN
        # print("#### in constrasint ####")
        # print(self.strct, self.sents_form, self.in_word_pos)
        if sents_form is not None:
            self.token_constraintor =  TokenConstraintor(
                strct=self.strct,
                in_word_pos=self.in_word_pos,
                sent_form=self.sents_form,
                tgt_dict=self.tgt_dict,
                eos=self.eos
            )
            self.order = 0 # To prevent tokens being generated out of order
            self.TOKEN = _config.TOKEN
    ...
    def _generate(
        ...
        for step in range(max_len + 1):  # one extra step for EOS marker
            ...
            if self.sents_form is not None:
                # print("USING CONSTRAINT")
                top_idx = torch.topk(lprobs, k=5, dim=1)
                pi = [ self.tgt_dict.string(k) for k in top_idx[1] ]
                flag = pi[0]

                if flag != '' and flag.split('_')[0] == self.TOKEN[self.order % len(self.TOKEN)]:
                    if  self.order % 4 == 1 and self.order > 1:
                        # 在 Pos 段处理乐句分段
                        lprobs = self.token_constraintor.pos_constraint(
                            lprobs,
                            tokens,
                            self.order,
                        )

                    elif self.order % 4 == 2 and self.order > 2:
                        # 在 Pitch 段处理倒字
                        lprobs = self.token_constraintor.pitch_constraint(
                            lprobs,
                            tokens,
                            self.order,
                        )
        ...
        if self.sents_form is not None:
            self.order = 0
            self.token_constraintor.print_debug()  
