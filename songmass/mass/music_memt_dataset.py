...
# 和原来MASS基本一样，修改的地方是noisy_sent如果是旋律序列,pitch换pitch，duration换duration，然后返回两个sent_id序列和两个word_id序列
#两个id序列里, -1和-2分别标识source和target的padding
...
class MusicMemtDataset(FairseqDataset):
    """
    ...
    def noisy_sent(self, lst, vocab, lang):
        if lang == 'lyric':
            ret = []
            for i, w in enumerate(lst):
                p = np.random.random()
                if p <= self.ratio:
                    ret.append(vocab.mask_index)
                else:
                    ret.append(w)
            return ret
...
#         print("tgt_list",tgt_list)
        ...
        for i,s in enumerate(sentences):
            ...
            sent = self.noisy_sent(sent, self.src_vocab, self.src_lang)
            source.extend(sent)        
...
#         print("target_word_ids",target_word_ids)
    ...
    def collate(
        ...
        source_word_ids = merge_sentId('source_word_ids', left_pad=self.left_pad_target,pad_idx=-1)
        ...
        batch = {
            ...
            'target': target,
            'word_ids' : {'source_word_ids':source_word_ids,'target_word_ids':target_word_ids}
        }
