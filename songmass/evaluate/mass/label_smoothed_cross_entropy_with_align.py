...
class LabelSmoothedCrossEntropyCriterionWithAlign(FairseqCriterion):
    ...
    def __init__(self, args, task):
        ...
        self.prob_outfile = args.prob_outfile
    ...
    def add_args(parser):
        ...
        # SZH
        parser.add_argument('--prob-outfile', type=str, metavar='STR',
                            help='path to output probs')
    ...
    def forward(self, model, sample, reduce=True):
        ...
        if self.prob_outfile is not None:
            # log false: 和为1
            probs = model.get_normalized_probs(net_output, log_probs=False).squeeze(0) # B * T * NumClass -> T * NumClasses (B = 1 For this part)
            ground_truth = sample['target'].squeeze(0).view(-1,1) # B * T -> T -> T*1
            probs = probs.gather(dim=-1,index=ground_truth) #T*1
            probs = probs.view(-1)
            with io.open(self.prob_outfile,'w')as f:
                f.write(str(probs.tolist()))
            exit(0)
    ...
    def compute_loss(self, model, net_output, sample, reduce=True):
        ...
        non_pad_mask = target.ne(self.padding_idx) #(B*T, 1)
        ...
        if not ('word_ids' in sample.keys() and 'attns' in net_output[1].keys()):
            ...
            true_word_attn = word_attn / attn_word_num #(B,T,S)
